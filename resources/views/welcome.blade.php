...
@section('content')
    <div
        class="bg-crave-beige min-h-[85vh] rounded-3xl p-8 md:p-16 flex flex-col items-center justify-center text-center relative overflow-hidden shadow-sm">
                    ...
                    @auth
                        @if (Auth::user()->role === 'seller' || Auth::user()->role === 'admin')
                            <a href="{{ route('products.index') }}"
                                class="bg-crave-lime text-crave-teal px-10 py-4 rounded-full font-bold hover:bg-crave-green transition-colors shadow-lg text-lg">
                                My Products
                            </a>
                        @endif
                    @else
                        <a href="{{ route('login') }}"
                            class="bg-crave-teal text-white px-10 py-4 rounded-full font-bold hover:bg-crave-darkgreen transition-colors shadow-lg text-lg">
                            Log in
                        </a>
