    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('addresses.create') }}"
                class="bg-crave-green hover:bg-crave-darkgreen text-white px-5 py-2.5 rounded-full font-medium transition-colors flex items-center shadow-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @forelse($addresses as $address)
                <div
                    class="bg-white rounded-2xl p-6 shadow-sm border {{ session('selected_address_id') == $address->Address_ID ? 'border-crave-lime ring-2 ring-crave-lime/20' : 'border-gray-100' }} hover:border-crave-lime transition-all relative">
                        ...
                        <div class="flex items-center space-x-2">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2">
                                ...
                                @if (session('selected_address_id') == $address->Address_ID)
                                    <span
                                        class="bg-crave-lime text-crave-teal text-[10px] font-extrabold px-2 py-0.5 rounded-full uppercase w-fit">Active</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <div class="flex items-center space-x-3 text-gray-400 shrink-0">
                            @if (session('selected_address_id') != $address->Address_ID)
                                <form action="{{ route('cart.selectAddress') }}" method="POST" class="inline">
                                    ...
                                    <button type="submit"
                                        class="text-xs bg-gray-100 text-gray-600 font-bold px-3 py-1.5 rounded-full hover:bg-crave-lime hover:text-crave-teal transition-colors">
                                        Use This
                                    </button>
                                </form>
                            @endif
                            ...
                            <a href="{{ route('addresses.edit', $address->Address_ID) }}"
                                class="hover:text-crave-teal transition-colors flex items-center" title="Edit Address">
                            ...
                            <form action="{{ route('addresses.destroy', $address->Address_ID) }}" method="POST"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="hover:text-crave-pink transition-colors flex items-center"
            ...
            @empty
                <div class="col-span-full bg-gray-50 rounded-2xl p-12 text-center border-2 border-dashed border-gray-200">
                    ...
                    <a href="{{ route('addresses.create') }}"
                        class="text-crave-green hover:underline text-sm mt-2 inline-block">Add your first address</a>
                </div>
            @endforelse
        </div>
    </div>
