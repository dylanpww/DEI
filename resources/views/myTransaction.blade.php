...
<div class="max-w-5xl mx-auto pb-20">
    ...
    @if($orders->isEmpty())
        <div class="bg-white rounded-3xl p-12 text-center shadow-sm border border-gray-100">
            ...
            <a href="{{ route('explore') }}" class="bg-crave-lime text-crave-darkgreen px-8 py-3 rounded-full font-bold hover:bg-crave-green transition-all">Mulai Jelajah</a>
        </div>
    @else
        <div class="space-y-6">
                ...
                <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex justify-between items-center border-b border-gray-100">
                        <div>
                            ...
                            <p class="text-sm font-semibold text-gray-700">{{ $order->created_at->format('d M Y, H:i') }}</p>
                        </div>
                            ...
                            @foreach($order->items as $item)
                                <div class="flex items-center gap-4">
                                    <div class="w-16 h-16 bg-crave-beige rounded-2xl flex-shrink-0 overflow-hidden">
                                        ...
                                        <img src="{{ asset('storage/'.$item->product->image) }}" class="w-full h-full object-cover" onerror="this.src='https://placehold.co/100x100?text=Food'">
                                    </div>
                                    <div class="flex-1">
