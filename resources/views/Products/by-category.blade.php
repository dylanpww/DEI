...
<div class="bg-white rounded-3xl shadow-sm p-8">
    <!-- Header -->
    <div class="mb-8">
        <a href="{{ route('explore') }}" class="text-gray-500 hover:text-crave-teal flex items-center gap-1 text-sm font-medium mb-4">
    ...
    <div class="mb-8">
        <form method="GET" action="{{ route('products.by-category', $category->category_id) }}" class="relative">
            ...
            <input type="text" name="search" placeholder="Search products..." value="{{ $search }}" class="w-full bg-gray-50 border border-gray-200 rounded-2xl py-4 pl-12 pr-4 outline-none text-md focus:ring-2 focus:ring-crave-lime focus:border-transparent transition-all shadow-inner">
        </form>
    </div>
    ...
    @if($products->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            ...
            <div class="bg-gray-50 rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow">
                <div class="h-48 bg-crave-beige flex items-center justify-center">
